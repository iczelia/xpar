...
#if defined(XPAR_X86_64)
  if (cpuflags == -1)
    cpuflags = xpar_x86_64_cpuflags();
  if (cpuflags & 1) {
    if (length >= 32767)
      return crc32c_32k_x86_64_sse42(crc, data, length);
    else
      return crc32c_small_x86_64_sse42(crc, data, length);
  }
  return crc32c_tabular(crc, data, length);
#elif defined(XPAR_AARCH64)
  if (cpuflags == -1)
    cpuflags = crc32c_aarch64_cpuflags();
  if (cpuflags)
    return crc32c_small_aarch64_neon(crc, data, length);
  return crc32c_tabular(crc, data, length);
#else
  return crc32c_tabular(crc, data, length);
#endif
