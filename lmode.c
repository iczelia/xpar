...
LEO_FORCE_INLINE void XORSummer_Add(XORSummer * summer, const void* src, const uint64_t bytes) {
#ifdef LEO_M1_OPT
  if (summer->Waiting)
  {
...
LEO_FORCE_INLINE void XORSummer_Finalize(XORSummer * summer, const uint64_t bytes) {
#ifdef LEO_M1_OPT
  if (summer->Waiting)
...
LEO_FORCE_INLINE bool ErrorBitfield_IsNeeded(const ErrorBitfield * bf, unsigned mip_level, unsigned bit)
{
  if (mip_level >= 8)
      return true;
  return 0 != (bf->Words[mip_level - 1][bit / 64] & ((uint64_t)1 << (bit % 64)));
}
...
void ErrorBitfield_Prepare(ErrorBitfield * bf)
{
  ...
  for (unsigned i = 0; i < kWords; i += 2)
      bf->Words[6][i] = bf->Words[6][i + 1] = bf->Words[5][i] | bf->Words[5][i + 1];
}
...
#ifdef LEO_ERROR_BITFIELD_OPT
  ErrorBitfield error_bits;  memset(&error_bits, 0, sizeof(error_bits));
#endif // LEO_ERROR_BITFIELD_OPT
