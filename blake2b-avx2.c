...
void blake2b_compress_avx2(blake2b_state * restrict s,
  ...
  const __m256i rot24 = _mm256_setr_epi8(
     3, 4, 5, 6, 7, 0, 1, 2, 11,12,13,14,15, 8, 9,10,
     3, 4, 5, 6, 7, 0, 1, 2, 11,12,13,14,15, 8, 9,10);
  const __m256i rot16 = _mm256_setr_epi8(
  ...
  __m256i row4 = _mm256_xor_si256(
    _mm256_loadu_si256((const __m256i *)&blake2b_IV[4]),
  ...
  #define G1(buf) do {                                            \
    ...
    row2 = _mm256_shuffle_epi8(_mm256_xor_si256(row2, row3),      \
                               rot24);                            \
  } while (0)

  #define G2(buf) do {                                            \
    row1 = _mm256_add_epi64(_mm256_add_epi64(row1, buf), row2);   \
    row4 = _mm256_shuffle_epi8(_mm256_xor_si256(row4, row1),      \
                               rot16);                            \
    row3 = _mm256_add_epi64(row3, row4);                          \
    { __m256i t = _mm256_xor_si256(row2, row3);                   \
      row2 = _mm256_xor_si256(_mm256_srli_epi64(t, 63),           \
                              _mm256_add_epi64(t, t)); }          \
  } while (0)
