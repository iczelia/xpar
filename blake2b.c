...
static inline void store64(void * restrict dst, u64 w) {
  ...
  p[0] = (u8)(w      ); p[1] = (u8)(w >>  8);
  p[2] = (u8)(w >> 16); p[3] = (u8)(w >> 24);
  p[4] = (u8)(w >> 32); p[5] = (u8)(w >> 40);
  p[6] = (u8)(w >> 48); p[7] = (u8)(w >> 56);
}
...
#if defined(XPAR_X86_64)
  #ifdef HAVE_AVX2
    if (xpar_leo_x86_64_cpuflags() & 1) return blake2b_compress_avx2;
  #endif
  #ifdef HAVE_SSE41
    if (xpar_x86_64_cpuflags() & 1)     return blake2b_compress_sse41;
  #endif
...
int blake2b_update(blake2b_state * restrict s,
  ...
  if (inlen > left) {
    ...
    while (inlen > BLAKE2B_BLOCKBYTES) {
      ...
      p += BLAKE2B_BLOCKBYTES; inlen -= BLAKE2B_BLOCKBYTES;
    }
  }
...
int blake2b_final(blake2b_state * restrict s,
                  void * restrict out, sz outlen) {
  ...
  for (int i = 0; i < 8; i++) store64(hash + i * 8, s->h[i]);
