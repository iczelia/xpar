...
static char * ex_find_data(ex * x, const xpar_vol * v, char ** basename,
  ...
  *damaged = false;
  ...
  if (xpar_vol_tag_match(named, v)) { *basename = xpar_strdup(v->name);  return named; }
...
static bool ex_vol_open(ex * x, const char * path) {
  ...
  if (x->vol_count == x->vol_cap) {
    ...
    x->vol = xpar_realloc(x->vol, x->vol_cap * sizeof *x->vol);
  }
  x->vol[x->vol_count++] = v;
  return true;
}
...
static void ex_keep_plain(ex * x, u8 * p) {
  if (x->plain_count == x->plain_cap) {
    ...
    x->plain = xpar_realloc(x->plain, x->plain_cap * sizeof *x->plain);
  }
...
static void ex_open_armoured(ex * x, const xpar_volimg * v) {
  ...
  FATAL_UNLESS(xpar_armour_check(&p) == NULL,
               "the armoured prologue names unusable parameters: %s", xpar_armour_check(&p));
  FATAL_UNLESS(plain_len <= (u64) (sz) -1 / 2 && arm_len <= (u64) (sz) -1,
               "the armoured region is too large for this host");
  /*  Zero-fill a missing tail as erasures.  */
  if (384 + arm_len > v->size) {
    u64 avail = v->size > 384 ? v->size - 384 : 0;
    u8 * pad = xpar_alloc_raw((sz) arm_len);
  ...
  xpar_gf_init();
  a = xpar_armour_new(&p);
  plain = xpar_alloc_raw((sz) plain_len ? (sz) plain_len : 1);
  ...
  if (x->arm_count == x->arm_cap) {
    ...
    x->arm = xpar_realloc(x->arm, x->arm_cap * sizeof *x->arm);
  }
  { ex_arm * m = &x->arm[x->arm_count++];
    ...
    m->done = xpar_calloc((sz) (m->frames ? m->frames : 1), 1); }
  /*  Decode when packets or the descriptor do not verify.  */
  if (!xpar_verify_packets_ok(plain, plain_len, x->key_loaded ? &x->key : NULL)
      || !ex_have_setd(x)) {
    u8 * region = xpar_alloc_raw((sz) arm_len ? (sz) arm_len : 1);
    u8 * fixed  = xpar_alloc_raw((sz) plain_len ? (sz) plain_len : 1);
...
static void ex_pick_setd(ex * x) {
  ...
  FATAL_UNLESS_CODE(XPAR_EXIT_UNREPAIRABLE,
                    want != NULL, "no set descriptor survived in '%s'",
                    x->o->set);
...
static void ex_read_manifest(ex * x) {
  ...
  Fi(x->sd.file_count,
    ...
    FATAL_UNLESS_CODE(XPAR_EXIT_UNREPAIRABLE,
                 ...
                 " is missing from every volume", i + 1, x->sd.file_count);
...
static void ex_apply_meta(ex * x, u32 idx, const char * path) {
  ...
  if (pr && (pr->uid != XPAR_ID_NONE || pr->gid != XPAR_ID_NONE ||
    ...
    if (!(o->preserve & XPAR_PRES_OWNER))
      ex_skip(x, e, EX_SK_OWNER, "--preserve=owner not given");
    else {
  ...
  { i64 at = XPAR_TIME_NONE, mt = XPAR_TIME_NONE, bt = XPAR_TIME_NONE;
    ...
    if ((o->preserve & XPAR_PRES_ATIME) && e->atime_ns != XPAR_ABSENT_TIME)
      ...
      ex_skip(x, e, EX_SK_ATIME, "--preserve=atime not given");
  ...
  if (pr && pr->xattr_count) {
    ...
    if (!(o->preserve & XPAR_PRES_XATTR))
      ex_skip(x, e, EX_SK_XATTR, "--preserve=xattr not given");
    else Fk(pr->xattr_count,
...
static bool ex_replace(ex * x, char * stage, const char * path) {
  ...
  if (had && !x->o->force) { why = "it exists without -f";  goto refuse; }
  if (had) {
    for (i = 0; i < 1000; i++) {
#if defined(XPAR_DOS) || defined(__MSDOS__)
    ...
    if (!backup) { why = "no backup name available";  goto refuse; }
...
static bool ex_write_entry(ex * x, u32 idx, const char * path) {
  ...
  { char * d = xpar_path_dir(path);
    if (xpar_mkdir_p(d, 0777) != 0) { xpar_free(d);  x->io_failures++;  return false; }
    xpar_free(d); }
    ...
    while (left) {
      ...
      if (!ex_read_stream(x, at, take, buf)) {
        ...
        FATAL_IO("the set stream is missing bytes [%" PRIu64 ", %" PRIu64
                 ") needed by '%.*s'; their data volume is absent", at,
  ...
  if (xpar_memcmp(got, e->content_hash, 32)) {
    ...
    xpar_remove(stage);
    ex_note(x, "xpar: '%.*s' does not match its recorded hash; not written; "
               "run 'xpar repair' first\n",
            (int) e->name_len, e->name);
    xpar_free(stage);
    return false;
  }
  if (!ex_replace(x, stage, path)) { xpar_free(stage);  return false; }
  ...
  { const char * why = "this filesystem has no links";
    if (x->caps & XPAR_FS_HARDLINK) {
      ...
      if (!stage) why = "no staging name is free";
        ...
        return;
      } else why = "cannot publish staged link";
      xpar_free(stage);
    }
    ex_skip(x, e, EX_SK_LINKCOPY, why); }
  ex_note(x, "xpar: %.*s: materialised as a copy\n", (int) e->name_len,
          e->name);
  x->copies++;
  xpar_free(src);
...
static void ex_free(ex * x) {
  u32 i;
  Fi(x->stream_count, xpar_vset_close(x->stream_set[i]));
  xpar_free(x->stream_set);
  if (x->have_chain) {
    Fi(x->chain.gen_count,
  ...
  Fi(x->arm_count, xpar_armour_free(x->arm[i].a);  xpar_free(x->arm[i].done));
  xpar_free(x->arm);  x->arm = NULL;  x->arm_count = x->arm_cap = 0;
  Fi(x->vol_count, xpar_volimg_close(&x->vol[i]));
  xpar_free(x->vol);
  Fi(x->plain_count, xpar_free(x->plain[i]));
  xpar_free(x->plain);
  Fi(x->posix_count,
    u32 k;
    Fk(x->posix[i].xattr_count,
...
static void ex_open_chain(ex * x) {
  ...
  xpar_nameidx_build(&x->mf, &x->nix);
  x->posix_tab = xpar_calloc(
    x->chain.gen_count ? x->chain.gen_count : 1, sizeof *x->posix_tab);
  x->posix_tab_count = xpar_calloc(
    x->chain.gen_count ? x->chain.gen_count : 1, sizeof *x->posix_tab_count);
  Fi(x->chain.gen_count,
    x->posix_tab_count[i] = xpar_gchain_posix(&x->chain, i,
       &x->posix_tab[i]));

  x->stream_set = xpar_calloc(walked, sizeof *x->stream_set);
  g = x->selected;
  while (g != XPAR_GEN_NONE) {
    xpar_options ro = *x->o;
    xpar_genref ref;
...
static void ex_validate(ex * x) {
  ...
  if (x->have_chain) {
    ...
    anc = xpar_calloc(n ? n : 1, sizeof *anc);
    ...
    for (k = 1; k < n; k++)
      FATAL_UNLESS(anc[k - 1].base + anc[k - 1].length <= anc[k].base,
                   "generation stream ranges overlap");
...
int xpar_op_extract(const xpar_options * o) {
  ...
  FATAL_UNLESS(o->set_ref.count > 0, "extract needs a set to work on");
  ...
  FATAL_UNLESS(x.vol_count > 0, "nothing in '%s' could be opened", o->set);
  if (!ex_have_setd(&x))
    Fi(x.vol_count, ex_salvage(&x, x.vol[i].data, x.vol[i].size));

  ex_pick_setd(&x);
  ex_authenticate(&x);
  FATAL_UNLESS(x.sd.layout != XPAR_LAYOUT_SIDECAR,
               "this set uses external files; use verify or repair");
  ...
  if (!x.stream_count && !x.strm) {
    FATAL_UNLESS_CODE(XPAR_EXIT_UNREPAIRABLE,
                      x.have_layt, "this set has no volume layout");
    ...
    Fi(x.layt.count,
      ...
      FATAL_UNLESS_CODE(XPAR_EXIT_UNREPAIRABLE,
                   ...
                   "stream; run 'xpar repair' to rebuild it", v->name);
  ...
  if (o->to_stdout) {
    ...
    xpar_blake3_t h;
    u8 got[32];
    Fi(x.mf.count, if (x.mf.entry[i].entry_type == XPAR_ENTRY_REGULAR) { regs++;  only = i; });
    FATAL_UNLESS(regs == 1, "--stdout requires one entry, but the set has %" PRIu32
                 "; extract to a directory", regs);
  ...
  FATAL_UNLESS(xpar_lstat(x.dest, &st) == 0 && !st.is_symlink,
               "extraction directory '%s' is a symbolic link; use its "
               "target path", x.dest);
  ...
  Fi(x.mf.count,
    ...
    if (p) {
      ...
      if (e->entry_type == XPAR_ENTRY_DIR)
        FATAL_UNLESS(!exists || st.is_dir,
                     "destination '%s' exists and is not a directory", p);
      else {
        FATAL_UNLESS(!exists || o->force,
...
summary:
  ...
  if (o->json) {
    ...
    ex_note(&x, "xpar: %" PRIu64 " %s, %" PRIu64 " bytes, %" PRIu64
            " link%s, %" PRIu64 " cop%s, %" PRIu64 " hash mismatch%s.\n",
            x.entries,
