...
static u64 packets_end(const u8 *, u64, const xpar_key *);
...
typedef struct {
  ...
  char *    plain_stage;
  u8 *      plain_mem;         /*  Set when the plaintext needs no stage.  */
  u8 *      data;
  u64       size;
  u64       pkt_dropped;  /*  Stored packets of this volume that failed.  */
  u64       pkt_stale;    /*  Packets outranked by the index.  */
  bool      is_index;     /*  Index-volume header.  */
  bool      armoured;
...
static void vimg_remap(xpar_vimg * v) {
  ...
  if (!f) {
    if (!xpar_errno_absent(xpar_errno()))
      FATAL_IO("cannot reopen '%s': %s", v->path,
               xpar_strerror(xpar_errno()));
    return;
  }
  n = xpar_size(f);
  ...
  if (got != (sz) n) {
    ...
    if (now < 0 || (u64) now != (u64) got)
      FATAL_IO("reading '%s' stopped after %" PRIu64 " of %" PRIu64
               " bytes", v->path, (u64) got, (u64) n);
    xpar_memset(v->data + got, 0, (sz) n - got);
  }
  v->size = (u64) got;
  xpar_xclose(f);
}
...
struct xpar_vset {
  ...
  xpar_critset crit;
  u64          pkt_dropped;   /*  Stored packets that failed a checksum.  */
  u32          scan_img;      /*  Image scan_into is reading, or img_count.  */
  xpar_key     key;
  ...
  /*  Archive length or prologue mismatch.  */
  bool          archive_resized, archive_prologue_bad;
  char *        dir;
  char * const * source;
  ...
  u64           io_errors;     /*  Host refusals on any path this run read.  */
  ...
  u64 link_drift;             /*  Entries whose recorded structure differs.  */
...
static void vset_io_error(xpar_vset * s, const char * path, int err) {
  s->io_errors++;
  xpar_fprintf(xpar_stderr, "xpar: cannot read '%s': %s\n", path,
               xpar_strerror(err));
}

u64 xpar_vset_io_errors(const xpar_vset * s) { return s->io_errors; }
...
static xpar_file * plain_stage_open(const char * archive, char ** path) {
  static const char * const var[] = { "TMPDIR", "TMP", "TEMP" };
  char * stem = NULL;
  xpar_file * f;
  /*  Try the host default after environment directories.  */
  u32 i;
  Fi(ARRAY_LEN(var) + 1,
    const char * dir = i < ARRAY_LEN(var) ? xpar_getenv(var[i])
                                          : xpar_tmpdir();
    if (!dir || !*dir) continue;
...
/*  Write plaintext to memory or its staging file.  */
static bool plain_put(xpar_vimg * v, const u8 * src, u64 len, u64 off) {
  if (v->plain_mem) {
    if (off > v->plain_len || len > v->plain_len - off) return false;
    xpar_memcpy(v->plain_mem + off, src, (sz) len);
    return true;
  }
  return v->plain_file &&
         xpar_pwrite(v->plain_file, src, (sz) len, off) == (sz) len;
}
...
static u8 * open_armoured_plain(xpar_vset * s, xpar_vimg * v,
  ...
  u8 * frame = NULL, * base;
  FATAL_UNLESS(fd <= s->memory_budget && fd <= (u64) (sz) -1,
               "the armoured frame needs %" PRIu64 " bytes but -m permits %"
               PRIu64 "; "
               "raise -m to read this archive",
               fd,
               s->memory_budget);
  FATAL_UNLESS(pr->plain_length != 0 &&
               (!frames || fd <= UINT64_MAX / frames) &&
               pr->armoured_length == frames * fd,
               "the armoured frame geometry is invalid");
  ...
  { u64 keep = MIN(avail, pr->armoured_length);
    ...
    if (keep < pr->armoured_length)
      xpar_memset(frame + keep, 0, (sz) (pr->armoured_length - keep));
  ...
  if (avail < pr->armoured_length &&
      !xpar_verify_packets_ok(base, pr->plain_length, NULL)) {
    u64 keep = packets_end(base, pr->plain_length, NULL);
    if (keep >= pr->stream_offset + pr->stream_length) {
      u64 done = keep;
...
static void open_armoured_image(xpar_vset * s, xpar_vimg * v) {
  ...
  ap.depth = pr.depth;
  FATAL_UNLESS(xpar_armour_check(&ap) == NULL,
               "the armoured prologue of '%s' names unusable parameters", v->path);
  /*  Require the prologue; length mismatches are repairable damage.  */
  FATAL_UNLESS(v->size >= 384,
               "the armoured region of '%s' is truncated", v->path);
  FATAL_UNLESS(pr.plain_length <= (u64) (sz) -1,
               "the armoured plaintext of '%s' is too large for this host", v->path);
  xpar_gf_init();
  a = xpar_armour_new(&ap);
  FATAL_UNLESS(a != NULL,
               "the armoured parameters of '%s' cannot be instantiated", v->path);
  plain = open_armoured_plain(s, v, a, &pr, &ast);
  if (ast == XPAR_ARMOUR_CORRECTED) s->armg_corrected++;
  xpar_armour_free(a);
  /*  Let the outer code repair a missing tail.  */
  v->short_tail = (u64) v->size < 384 + pr.armoured_length;
  if ((u64) v->size != 384 + pr.armoured_length) s->archive_resized = true;
  if (xpar_garm_prologue_copies(v->data, (sz) v->size) < 3)
    s->archive_prologue_bad = true;
  FATAL_UNLESS_CODE(XPAR_EXIT_UNREPAIRABLE,
               ast != XPAR_ARMOUR_FAILED || v->short_tail,
               "the armoured plaintext of '%s' is damaged past the inner "
               "code's capacity",
               v->path);
  FATAL_UNLESS(pr.stream_offset <= pr.plain_length &&
               pr.stream_length <= pr.plain_length - pr.stream_offset,
               "the STRM range in '%s' is outside its plaintext",
               v->path);
  v->armoured = true;
  v->plain = plain;
  v->plain_len = pr.plain_length;
...
bool xpar_vset_armour_correct(xpar_vset * s, u64 lo, u64 hi) {
  ...
  image = &s->img[s->archive_img];
  if ((!image->plain_file && !image->plain_mem) || !image->plain)
    return false;
  ...
  for (f = first; f <= last; f++) {
    ...
    if (!plain_put(image, plain, have, po)) break;
    s->armg_corrected++;
    changed = true;
  }
  xpar_free(enc);  xpar_free(plain);  xpar_armour_free(a);
...
/*  End of the leading run of well-formed packets.  */
static u64 packets_end(const u8 * p, u64 n, const xpar_key * key) {
  u64 pos = 0;
  while (n - pos >= XPAR_PKT_HDR) {
    ...
    if (st != XPAR_OK && st != XPAR_E_NEEDKEY) break;
    pos += h.length;
  }
  return pos;
}
...
u8 * xpar_verify_armg_plain(const u8 * body, sz n, const xpar_key * key,
  ...
  *out_len = 0;
  ...
  xpar_armour_free(ar);
  if (rc == XPAR_ARMOUR_FAILED) { xpar_free(plain);  return NULL; }
...
static bool vimg_is_index(const xpar_vimg * v, const xpar_key * key) {
  xpar_pkt h;
  xpar_volh vh;
  if (v->size < XPAR_PKT_HDR || xpar_memcmp(v->data, XPAR_PKT_MAGIC, 8))
    return false;
  if (xpar_pkt_read(v->data, v->size, key, &h) != XPAR_OK ||
      !xpar_pkt_is(&h, XPAR_T_VOLH) ||
      xpar_volh_read(v->data + XPAR_PKT_HDR, (sz) (h.length - XPAR_PKT_HDR),
                     &vh) != XPAR_OK)
    return false;
  return vh.volume_kind == XPAR_VOL_INDEX;
}
...
static void collect_gens(xpar_vset * s) {
  ...
  for (i = 1; i < n; i++) {
    ...
    u32 k = i;
    while (k && s->gen[k - 1].generation < t.generation) { s->gen[k] = s->gen[k - 1];  k--; }
    s->gen[k] = t;
  }
...
static void list_branches(const xpar_vset * s) {
  u32 i, j;
  Fi(s->gen_count,
    char id[XPAR_SET_ID_LEN * 2 + 1];
    bool named = false;
    Fj(s->gen_count, if (s->gen[j].parent == i) { named = true;  break; });
    if (named) continue;
    xpar_hex(id, s->gen[i].id, XPAR_SET_ID_LEN);
    xpar_fprintf(xpar_stderr,
                 "xpar:   branch head generation %" PRIu32 ": --generation=%s\n",
                 s->gen[i].generation, id));
}
...
static void validate_identities(xpar_vset * s) {
  ...
  FATAL_UNLESS(setp != NULL, "the selected set descriptor disappeared during identity "
               "validation");
  ...
  Fi(s->setd.file_count,
    ...
    FATAL_UNLESS(fp != NULL, "manifest entry %" PRIu32 " disappeared during set identity "
                 "validation", i);
...
static void load_tables(xpar_vset * s) {
  u64 input = 0;
  u32 i, cps;
  Fi(s->img_count, input += s->img[i].size);
...
static char * find_renamed_volume(xpar_vset * s, u32 index, u32 kind,
  ...
  *found_name = NULL;
  if (!d) {
    if (!xpar_errno_absent(xpar_errno()))
      vset_io_error(s, s->dir, xpar_errno());
    return NULL;
  }
...
static void load_layt(xpar_vset * s) {
  ...
  if (s->have_layt) {
    ...
    Fi(s->layt.count,
      ...
      err = xpar_lstat(path, &vst) == 0 ? 0 : xpar_errno();
      if (err && !xpar_errno_absent(err)) vset_io_error(s, path, err);
      if (err || !vst.is_regular) {
    ...
    Fi(s->layt.count,
      ...
      err = xpar_lstat(path, &st) == 0 ? 0 : xpar_errno();
      if (err && !xpar_errno_absent(err)) vset_io_error(s, path, err);
      if (err || !st.is_regular || st.size < minimum) {
        char * found_name = NULL;
    ...
    if ((s->pkt_dropped || s->armg_failed) && s->recovery) {
      ...
      Fi(s->crit.count,
        if (xpar_pkt_is(&s->crit.pkt[i].hdr, XPAR_T_RCVS) &&
            !xpar_memcmp(s->crit.pkt[i].hdr.set_id, s->set_id,
                         XPAR_SET_ID_LEN)) seen++);
      if (seen < s->recovery) s->recovery_bad = s->recovery - seen;
    }
    return;
  }
...
static void split_score_candidate(xpar_vset * s, const xpar_vol * lv,
  ...
  xpar_stat_t st;
  u64 tested, score;
  if (xpar_lstat(path, &st) != 0 || !st.is_regular || !st.size) return;
  if (split_claimed(s, lv, name)) return;
  score = split_volume_score(s, lv, path, &tested);
  if (!tested) score = 0;
  /*  Reject files refused by every certificate.  */
  if (tested && !score) return;
  /*  Without certificates, require the recorded length.  */
  if (!tested && st.size != lv->byte_length) return;
  (*candidates)++;
...
static char * find_split_volume(xpar_vset * s, const xpar_vol * lv,
                                char ** found_name) {
  char * expected = xpar_path_vol(s->dir, lv->name);
  ...
  *found_name = NULL;
  ...
  expected = xpar_path_vol(s->dir, lv->name);
  ...
  xpar_free(best_path);  xpar_free(best_name);
  /*  Treat a named, wrong-length file as damaged rather than missing.  */
  expected = xpar_path_join(s->dir, lv->name);
  if (xpar_lstat(expected, &st) == 0 && st.is_regular && st.size) {
    *found_name = xpar_strdup(lv->name);
    return expected;
  }
  xpar_free(expected);
...
static void load_owned_data(xpar_vset * s) {
  ...
  FATAL_UNLESS(s->have_layt, "a split set needs a volume layout");
  ...
  Fi(s->layt.count,
    ...
    if (path) {
      ...
      if (!xpar_path_same(lv->name, found_name)) {
        ...
        /*  Record a wrong-length volume for repair.  */
        if (xpar_lstat(path, &st) == 0 && st.size != lv->byte_length)
          subst_add(s, lv->name, lv->name, i);
        xpar_free(found_name);
      }
      xpar_free(path);
    });
...
/*  Find intact replica bytes matching a damaged packet.  */
static const u8 * replica_bytes(const xpar_vset * s, u32 skip,
                                const u8 * dmg, u64 len) {
  const xpar_key * key = s->keyed ? &s->key : NULL;
  u32 j;
  Fj(s->img_count,
    xpar_scan sc;  xpar_pkt h;  const u8 * body;  u64 off;
    if (j == skip || s->img[j].armoured || !s->img[j].data) continue;
    xpar_scan_init(&sc, s->img[j].data, s->img[j].size, key, false);
    sc.accept_unverified_keyed = false;
    while (xpar_scan_next(&sc, &h, &body, &off))
      if (h.length == len &&
          !xpar_memcmp(body - XPAR_PKT_HDR + 40, dmg + 40, 8))
        return body - XPAR_PKT_HDR);
  return NULL;
}

/*  Preserve the first failure reason.  */
static void first_reason(const char ** reason, const char * why) {
  if (reason && !*reason) *reason = why;
}

/*  Replace failed packets from replicas, then publish the volume.  */
static bool rewrite_dropped_image(xpar_vset * s, u32 idx,
                                  const char ** reason) {
  const xpar_key * key = s->keyed ? &s->key : NULL;
  xpar_vimg * v = &s->img[idx];
  xpar_file * f;
  char * stage = NULL;
  u8 * copy;
  u64 pos = 0;
  bool ok = true;
  if (!v->data || v->size < XPAR_PKT_HDR || v->size > (u64) (sz) -1) {
  ...
  while (pos <= v->size - XPAR_PKT_HDR) {
    xpar_pkt h;
    xpar_status st;
    if (xpar_memcmp(copy + pos, XPAR_PKT_MAGIC, 8)) { pos += XPAR_PKT_ALIGN;  continue; }
    st = xpar_pkt_read(copy + pos, v->size - pos, key, &h);
    if (st == XPAR_OK) { pos += h.length;  continue; }
    if (st == XPAR_E_CHECKSUM) {
      const u8 * raw = replica_bytes(s, idx, copy + pos, h.length);
      if (!raw) {
        first_reason(reason, "no other volume holds an intact replica");
        ok = false;  break;
      }
      xpar_memcpy(copy + pos, raw, (sz) h.length);
  ...
  if (ok) {
    xpar_scan sc;  xpar_pkt h;  const u8 * body;  u64 off;
    xpar_scan_init(&sc, copy, v->size, key, false);
    sc.accept_unverified_keyed = false;
    while (xpar_scan_next(&sc, &h, &body, &off)) { }
    if (sc.skip_checksum) {
      first_reason(reason, "the rewritten volume still fails a checksum");
      ok = false;
    }
  }
  if (ok) {
...
bool xpar_vset_trim_ragged(xpar_vset * s, u64 * volumes, u64 * failures,
                           const char ** reason) {
  u32 i;
  u64 done = 0, failed = 0;
...
u64 xpar_vset_volumes_stale(const xpar_vset * s) {
  ...
  u32 i;
  Fi(s->img_count,
    if (s->img[i].pkt_stale && !s->img[i].armoured) n++);
  return n;
}
...
u64 xpar_vset_volumes_dropped(const xpar_vset * s) {
  ...
  u32 i;
  Fi(s->img_count,
    if (s->img[i].pkt_dropped && !s->img[i].armoured) n++);
  return n;
}

bool xpar_vset_rewrite_dropped(xpar_vset * s, u64 * volumes, u64 * failures,
                               const char ** reason) {
  u32 i;
  u64 done = 0, failed = 0;
  if (volumes) *volumes = 0;
  ...
  if (reason) *reason = NULL;
  Fi(s->img_count,
    if (!s->img[i].pkt_dropped || s->img[i].armoured) continue;
    if (rewrite_dropped_image(s, i, reason)) done++;
    else failed++);
  if (volumes) *volumes = done;
  if (failures) *failures = failed;
  return failed == 0;
...
bool xpar_vset_restore_names(xpar_vset * s, u64 * volumes, u64 * failures,
  ...
  Fi(s->subst_count,
    ...
    found = src ? (i64) src->size : xpar_size(in);
    if (found <= 0) {
...
bool xpar_vset_rewrite_substituted(xpar_vset * s, u64 * rewritten,
                                   u64 * relengthed, u64 * failures,
                                   const char ** reason) {
  u32 i;
  u8 * buf;
  u64 wrote = 0, sized = 0, failed = 0;
  if (reason) *reason = NULL;
...
typedef struct {
  ...
  u8 * buf;
  const char * path;
  bool io_seen;               /*  Report a refused probe read once.  */
} v_confirm;
...
static bool v_confirm_at(void * user, u32 at, u64 physical) {
  ...
  if (xpar_pread(c->f, c->buf, (sz) z, physical) != (sz) z) {
    if (xpar_error(c->f) && !c->io_seen) {
      c->io_seen = true;
      vset_io_error(c->s, c->path, xpar_error(c->f));
    }
    return false;
  }
  if (c->s->keyed)
...
static void v_resync_entry(xpar_vset * s, u32 entry,
  ...
  if (xpar_error(f) && !confirm.io_seen) {
    confirm.io_seen = true;
    vset_io_error(s, path, xpar_error(f));
  }
  ...
  if (got.clipped && !o->quiet)
    xpar_fprintf(xpar_stderr, "xpar: %s: %" PRIu64 " matches lie outside "
                 "--resync-window; raise it to consider them\n",
                 path, got.clipped);
  if (displaced && !o->quiet)
    xpar_fprintf(xpar_stderr, "xpar: %s: found %" PRIu32
                 " displaced slices (%" PRIu64 " confirmations)\n",
                 path, displaced, got.confirmations);
  else if (!displaced && map->count && o->verbose && !o->quiet)
    xpar_fprintf(xpar_stderr, "xpar: %s: resync: no displacement\n", path);
  xpar_free(located);
...
static void authenticate_armoured_images(xpar_vset * s) {
  ...
  Fi(s->img_count,
    ...
    FATAL_UNLESS(a != NULL, "the authenticated armoured parameters of '%s' cannot be "
                 "instantiated", v->path);
    fp = xpar_armour_frame_plain(a);
    fd = xpar_armour_frame_disk(a);
    frames = xpar_ceil_div(v->plain_len, fp);
    /*  Decode the region in one batch, zero-filling a short tail.  */
    { u64 region_len = frames * fd;
      u64 avail = v->size > 384 ? (u64) v->size - 384 : 0;
      ...
      xpar_memset(&sc, 0, sizeof sc);
      syndromes++;
      st = xpar_armour_decode_frames(a, frame, frames, &sc);
      if (st == XPAR_ARMOUR_FAILED) failed = true;
    ...
    FATAL_UNLESS_CODE(XPAR_EXIT_UNREPAIRABLE,
                      !failed || v->short_tail,
                      "the authenticated metadata in '%s' is damaged past "
                      "the inner code's capacity", v->path);

    /*  Refresh both aliases over the corrected plaintext.  */
    Fj(s->archive_plain, if (s->plain[j] == v->plain) break);
    v->plain = plain_base(v);
    if (j < s->archive_plain) s->plain[j] = v->plain;
    FATAL_UNLESS(xpar_verify_packets_ok(v->plain, v->plain_len, &s->key),
                 "authenticated metadata in '%s' exceeds inner-code "
                 "recovery",
                 v->path);
    if (corrected) s->armg_corrected++);
}
...
xpar_vset * xpar_vset_open(const xpar_options * o) {
  ...
  /*  Scan authoritative archives and index volumes first.  */
  s->scan_img = s->img_count;
  s->crit.rank = 2;
  Fi(s->archive_plain, scan_into(s, s->plain[i], s->plain_len[i], false, false));
  Fi(s->img_count,
    if (!s->img[i].armoured)
      s->img[i].is_index = vimg_is_index(&s->img[i],
                                         s->keyed ? &s->key : NULL));
  { u32 pass;
    for (pass = 0; pass < 2; pass++)
      Fi(s->img_count,
        u32 stale = s->crit.stale;
        if (s->img[i].armoured || (pass == 0) != s->img[i].is_index)
          continue;
        s->crit.rank = s->img[i].is_index ? 2 : 1;
        s->scan_img = i;
        scan_into(s, s->img[i].data, s->img[i].size,
                  o->resync == XPAR_RESYNC_ALWAYS, false);
        s->img[i].pkt_stale = s->crit.stale - stale;
        /*  A volume the packets do not tile exactly is nonconforming.  */
        s->img[i].pkt_end = volume_packets_end(s->img[i].data,
                                               s->img[i].size,
                                               s->keyed ? &s->key : NULL);
  ...
  if (s->crit.stale && !o->quiet && !o->json)
    xpar_fprintf(xpar_stderr, "xpar: stale critical packets in %" PRIu64
                 " volume%s; run repair\n",
                 xpar_vset_volumes_stale(s),
                 PLURAL(xpar_vset_volumes_stale(s)));
  /*  Replicas keep the set readable, but the source volume is stale.  */
  if (s->pkt_dropped && !o->quiet && !o->json)
    xpar_fprintf(xpar_stderr, "xpar: %" PRIu64 " stored packet%s failed "
                 "checksum; replicas used; run repair\n", s->pkt_dropped,
  ...
  if (s->degraded || !s->geom.cell_bytes) { s->eg.cell_bytes = 0;  s->eg.cells_per_slice = 1; }
...
void xpar_vset_close(xpar_vset * s) {
  ...
  Fi(s->plain_count, if (s->plain_owned[i]) xpar_free(s->plain[i]));
  xpar_free(s->plain);
...
bool xpar_verify_written_volume(const char * path, const xpar_key * key,
  ...
  i64 signed_size;
  if (!f) FATAL_IO("cannot read back '%s': %s", path,
                   xpar_strerror(xpar_errno()));
  signed_size = xpar_size(f);
  if (signed_size < 0) FATAL_IO("cannot size '%s' for read-back", path);
...
u64 xpar_vset_volumes_to_rewrite(const xpar_vset * s) {
  ...
  u32 i;
  Fi(s->subst_count,
    if (xpar_strcmp(s->subst[i].want, s->subst[i].got)) n++);
  return n;
}
/*  Same-name entries record a length to restore, not a substitution.  */
u64 xpar_vset_volumes_to_relength(const xpar_vset * s) {
  u64 n = 0;
  u32 i;
  Fi(s->subst_count,
    if (!xpar_strcmp(s->subst[i].want, s->subst[i].got)) n++);
...
bool xpar_vset_archive_stale(const xpar_vset * s) {
  return s->archive_resized || s->archive_prologue_bad;
}
...
static xpar_file * entry_handle(xpar_vset * s, u32 entry) {
  ...
  if (!s->fh) {
    if (!xpar_errno_absent(xpar_errno()))
      vset_io_error(s, path, xpar_errno());
    xpar_free(path);
    return NULL;
  }
  xpar_free(path);
...
bool xpar_vset_read(xpar_vset * s, u64 off, u8 * buf, u64 len) {
  ...
  while (len) {
    ...
    if (!f || xpar_pread(f, buf, (sz) take, physical) != take) {
      if (f && xpar_error(f)) {
        char * p = entry_path(s, sp.entry);
        vset_io_error(s, p, xpar_error(f));
        xpar_free(p);
      }
      xpar_memset(buf, 0, (sz) take);
      ok = false;
...
static void flush_slice(stream_acc * a, u64 slice) {
  ...
  if (s->keyed) {
    ...
    if (tag_bad || ignored) {
      bool local = false;
      for (col = 0; col < s->eg.cells_per_slice; col++)
        if (a->cell_susp[col]) { xpar_cell_mark(&s->er, slice, col);  local = true; }
  ...
  if (a->slice_bad) { s->bad_slices++;  if (!whole_bad) note_slice(a, slice, "cell"); }
  a->slice_crc = 0;
  a->slice_bad = false;
  if (s->keyed) xpar_blake3_init_keyed(&a->tag, s->key.k_slice);
  else          xpar_blake3_init(&a->tag);
...
static void acc_zero(stream_acc * a, u64 n) {
  while (n) { u64 take = MIN(n, (u64) sizeof zeros);  acc_feed(a, zeros, take);  n -= take; }
}
...
static bool hash_range(xpar_vset * s, xpar_file * f, u64 off, u64 n,
                       xpar_blake3_t * h, u8 * buf, xpar_pool * pool) {
  while (n) {
    sz take = (sz) MIN(n, (u64) s->io_span);
    if (xpar_pread(f, buf, take, off) != take) {
      if (xpar_error(f)) return false;
      xpar_memset(buf, 0, take);
    }
...
static void check_link(xpar_vset * s, u32 i, const char * path,
  ...
  if (st->size != te->length) { r->wrong_length = true;  return; }
  if (xpar_fs_caps(path) & XPAR_FS_LINKID) {
    char * tpath = entry_path(s, (u32) tgt);
    xpar_stat_t ts;
    int ok = xpar_lstat(tpath, &ts);
    xpar_free(tpath);
    if (ok == 0) { if (ts.dev != st->dev || ts.ino != st->ino) r->link_broken = true;  return; }
  }
  ...
  if (!hash_range(s, f, 0, te->length, &h, buf, pool)) {
    r->unreadable = true;  r->exists = false;
    vset_io_error(s, path, xpar_error(f));
    xpar_close(f);
    return;
  }
...
static void check_entry(xpar_vset * s, u32 i, stream_acc * acc,
  ...
  if (!r->exists && !xpar_errno_absent(xpar_errno())) {
    r->unreadable = true;
    vset_io_error(s, path, xpar_errno());
    xpar_free(path);
    return;
...
io_stop:
  xpar_free(path);
  if (f) xpar_close(f);
  if (!hashing || r->unreadable) return;
  { u8 got[XPAR_BLAKE3_OUT_LEN];
    xpar_blake3_final(&h, got, XPAR_BLAKE3_OUT_LEN);
...
static bool has_alias(const xpar_vset * s, u32 i) {
  const xpar_entry * e = &s->mf.entry[i];
  u32 k;
  Fk(e->extent_count, if (aliased(s, i, k)) return true);
  return false;
}
...
/*  Return the oldest ancestor owning an entry's extents.  */
static u32 entry_inherited(const xpar_vset * s, u32 i) {
  const xpar_entry * e = &s->mf.entry[i];
  u64 lo = s->geom.stream_base, hi = lo + s->geom.stream_length;
  u32 best = XPAR_GEN_NONE, k, g;
  Fk(e->extent_count,
    u64 off = e->extents[k].stream_offset;
    u64 end = off + e->extents[k].length;
    if (off >= lo && end <= hi) continue;
    for (g = 0; g < s->gen_count; g++)
      if (off >= s->gen[g].base && off - s->gen[g].base < s->gen[g].len &&
          (best == XPAR_GEN_NONE || s->gen[g].generation < best))
        best = s->gen[g].generation);
  return best;
}
...
int xpar_vset_check(xpar_vset * s, const xpar_options * o,
  ...
  Fi(s->mf.count, inherit[i] = XPAR_GEN_NONE);
  xpar_nameidx_build(&s->mf, &nix);
  if (o->fast) total = s->geom.stream_length;
  else Fi(s->mf.count, total += s->mf.entry[i].length);
  ...
  /*  Bound checksum work by stored bytes and the padded manifest size.  */
  {
    ...
    entry_cap = 0;
    Fi(s->mf.count,
      u64 next = entry_cap + s->mf.entry[i].length;
      entry_cap = next < entry_cap ? UINT64_MAX : next);
    /*  Allow one aligned slice per entry plus final padding.  */
    pad = z > UINT64_MAX / (s->mf.count + 2) ? UINT64_MAX
                                             : (u64) (s->mf.count + 2) * z;
    entry_cap = entry_cap > UINT64_MAX - pad ? UINT64_MAX : entry_cap + pad;
    cap = present > UINT64_MAX - z ? UINT64_MAX : present + z;
    if (entry_cap > cap) cap = entry_cap;
    if (sweep > cap)
  ...
  Fi(s->mf.count,
    ...
    /*  Hard links inherit their targets' ownership.  */
    if (e->entry_type == XPAR_ENTRY_HARDLINK) {
      i64 t = xpar_link_target(&s->mf, &nix, i);
      if (t >= 0) inherit[i] = entry_inherited(s, (u32) t);
    } else inherit[i] = entry_inherited(s, i);
    check_entry(s, i, &acc, o, &nix, buf, &r, &pg, pool);
    damaged = !r.exists || r.unreadable || r.wrong_length || r.hash_bad ||
              r.prefix_bad || r.link_broken;
    if (r.unreadable) s->unreadable_entries++;
    else if (!r.exists) s->missing_entries++;
    if (r.link_broken) s->link_drift++;
    if (damaged) s->bad_entries++;
    /*  Defer alias-local classification until the erasure table is final.  */
    if (damaged && r.exists && !r.unreadable && !r.wrong_length &&
        !r.link_broken)
      shape[i] = 1;
    if (js) {
      xpar_json_begin(js, "file_result");
...
scanned:
  /*  Count superseded bytes as decoder erasures.  */
  if (s->ignored_cell) {
    u64 sl;
    u32 col;
    for (sl = 0; sl < s->eg.slice_count; sl++) {
      bool any = false;
      for (col = 0; col < s->eg.cells_per_slice; col++) {
        if (!s->ignored_cell[sl * s->eg.cells_per_slice + col] ||
            xpar_cell_bad(&s->er, sl, col)) continue;
        xpar_cell_mark(&s->er, sl, col);
        s->gone_cells++;
        any = true;
      }
  ...
  Fi(s->mf.count,
    ...
    if (inherit[i] != XPAR_GEN_NONE) {
      if (!s->inherited_bad || inherit[i] < s->inherited_gen)
        s->inherited_gen = inherit[i];
      s->inherited_bad++;
    }
    else if (has_alias(s, i)) s->alias_bad++;
    else s->opaque_bad++);
  xpar_free(shape);  xpar_free(inherit);
  xpar_free(buf);
  xpar_pool_destroy(pool);
  ...
  rc = XPAR_EXIT_OK;
  /*  Superseded cells need no repair.  */
  if (s->er.bad_count > s->gone_cells || s->bad_entries)
  ...
  if (xpar_vset_volumes_stale(s) && rc == XPAR_EXIT_OK)
    rc = XPAR_EXIT_REPAIRABLE;
  ...
  if (s->unreadable_entries || s->io_errors) rc = XPAR_EXIT_IO;
  return rc;
}
...
void xpar_vset_report(const xpar_vset * s, const xpar_options * o,
  ...
  if (s->gone_slices)
    xpar_fprintf(xpar_stderr,
                 "xpar: %" PRIu64 " slice%s of generation %" PRIu32 " %s "
                 "superseded by a later generation and count as erasures; "
                 "consolidate to restore full protection\n",
                 s->gone_slices, PLURAL(s->gone_slices),
                 s->setd.generation,
                 s->gone_slices == 1 ? "is" : "are");
  ...
  xpar_fprintf(xpar_stderr,
               ...
               (s->er.bad_count - s->gone_cells),
               PLURAL(s->er.bad_count - s->gone_cells),
               s->depth);
  ...
  if (s->inherited_bad) {
    xpar_fprintf(xpar_stderr,
                 "xpar: %" PRIu64 " %s damaged in data inherited from "
                 "generation %" PRIu32 "\n",
                 s->inherited_bad,
                 s->inherited_bad == 1 ? "entry is" : "entries are",
                 s->inherited_gen);
    if (o->gen_count && !o->chain_member)
      xpar_fputs("xpar: run without --generation, or with --chain, to "
                 "check the owning generations\n", xpar_stderr);
  }
...
void xpar_vset_json_summary(const xpar_vset * s, xpar_json * js,
  ...
  xpar_json_u64(js, "cells_bad", s->er.bad_count - s->gone_cells);
  ...
  xpar_json_u64(js, "entries_inherited_damaged", s->inherited_bad);
  ...
  xpar_json_u64(js, "volumes_stale", xpar_vset_volumes_stale(s));
  xpar_json_u64(js, "volumes_dropped", xpar_vset_volumes_dropped(s));
  xpar_json_u64(js, "packets_dropped", s->pkt_dropped);
  xpar_json_u64(js, "recovery_gone", s->recovery_gone);
  xpar_json_u64(js, "recovery_bad", s->recovery_bad);
  xpar_json_u64(js, "entries_missing", s->missing_entries);
  xpar_json_u64(js, "link_drift", s->link_drift);
  xpar_json_bool(js, "archive_resized", s->archive_resized);
  xpar_json_bool(js, "archive_prologue_bad", s->archive_prologue_bad);
  xpar_json_bool(js, "degraded", s->degraded);
  xpar_json_u64(js, "entries_unreadable", s->unreadable_entries);
  xpar_json_u64(js, "read_failures", s->io_errors);
...
int xpar_op_verify(const xpar_options * o) {
  ...
  FATAL_UNLESS(!o->from_stdin, "verify cannot read a pipe; use --spool");
  FATAL_UNLESS(!(o->fast && o->strong),
               "options --fast and --strong are mutually exclusive");

  /*  Verify unselected ancestry.  */
  if (!walk && !o->gen_count) {
    s = xpar_vset_open(o);
    if (xpar_vset_setd(s)->generation) { xpar_vset_close(s);  s = NULL;  walk = true; }
  }
